...
class Ec2ClientBehaviorIntegrationTest {
  ...
  @Test
  fun `deleteVpc should throw same exception for non-existent VPC`() = runTest {
    val vpcId = "vpc-nonexistent-${Uuid.random()}"

    val actual = shouldThrow<ServiceException> { stub.deleteVpc { this.vpcId = vpcId } }
    val expected = shouldThrow<ServiceException> { ec2Client.deleteVpc { this.vpcId = vpcId } }
    actual.shouldBeEquivalentTo(expected)
  }
}
