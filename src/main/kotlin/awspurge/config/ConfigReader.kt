...
class ConfigReader {
    fun readConfig(file: Path): Config {
        val fileContent = SystemFileSystem.source(file).use { source ->
    ...
    fun readConfig(config: String): Config {
        ...
        val filterDefinitions =
            yamlNode.yamlMap.get<YamlMap>("filterDefinitions")?.entries?.map { filterDefinitionNode ->
                val filterName = filterDefinitionNode.key.content
                val filters = filterDefinitionNode.value.yamlList.items.map { typeFilterNode ->
                    parseFilter(typeFilterNode)
                }
                filterName to filters
            }?.toMap() ?: emptyMap()
        val accounts = yamlNode.yamlMap.get<YamlMap>("accounts")?.entries?.map { accountNode ->
            val accountId = accountNode.key.content
            val excludeFilters = accountNode.value.yamlMap.get<YamlList>("excludeFilters")?.items?.map {
                parseTypeFiltersOrResolveReference(it, filterDefinitions)
            }?.flatten() ?: emptyList()
            val role = accountNode.value.yamlMap.get<YamlScalar>("assumeRole")?.content
            require(role != null) { "No assumeRole found for account $accountId" }
            Config.AccountConfig(accountId, role, excludeFilters)
        }
        val resourceTypes = yamlNode.yamlMap.get<YamlMap>("resourceTypes")?.let { resourceTypesNode ->
    ...
    private fun parseTypeFiltersOrResolveReference(
        it: YamlNode,
        filterDefinitions: Map<String, List<Filter>>
    ) = if (it is YamlScalar) {
        filterDefinitions[it.content]
            ?: throw IllegalArgumentException("Filter definition ${it.content} not found")
    } else {
        listOf(parseFilter(it))
    }

    private fun parseFilter(filterNode: YamlNode): Filter = when {
        ...
        filterNode.yamlMap.entries.size == 1 && filterNode.yamlMap.entries.values.first() is YamlList -> {
            ...
            val filters = filterNode.yamlMap.entries.values.first().yamlList.items.map{ childFilterNode -> parseFilter(childFilterNode) }
