...
class ConfigReader {
    ...
    fun readConfig(config: String): Config {
        ...
        val accounts = yamlNode.yamlMap.get<YamlMap>("accounts")?.entries?.map { accountNode ->
            ...
            val accountConfig =
                accountNode.value as? YamlMap ?: return@map Config.AccountConfig(accountId, null, emptyList())
            val excludeFilters = accountConfig.get<YamlList>("excludeFilters")?.items?.map {
